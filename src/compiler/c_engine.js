...
export function lexer(code) {
    ...
    const keywords = [
        ...
        'break', 'continue', 'goto', 'struct', 'typedef', 'sizeof', 'printf', 'scanf'
    ];
    const multiCharOps = ['==', '!=', '<=', '>=', '&&', '||', '++', '--', '+=', '-=', '*=', '/=', '%=', '->', '<<', '>>'];
    const singleCharOps = ['+', '-', '*', '/', '=', '<', '>', '!', '%', '&', '|', '^', '~', '?', ':'];
    const punctuation = ['(', ')', '{', '}', '[', ']', ';', ',', '.', '#'];
    ...
    while (i < code.length) {
        ...
        if (char === "'") {
            ...
            if (code[i] === "'") {
                charLit += "'"; i++; col++;
                tokens.push({ type: 'CHAR_LITERAL', lexeme: charLit, line, col: startCol });
                continue;
            }
        }
...
class Parser {
    ...
    expectLex(lex) { const t = this.matchLex(lex); if (!t) throw new CompilerError(`Expected '${lex}'`, this.peek()?.line||'EOF', this.peek()?.col||'EOF'); return t; }
    expectType(type) { const t = this.matchType(type); if (!t) throw new CompilerError(`Expected ${type}`, this.peek()?.line||'EOF', this.peek()?.col||'EOF'); return t; }
    ...
    parse() {
        ...
        while (this.pos < this.tokens.length) {
            ...
            if (t.type === 'PREPROCESSOR') {
                ...
                root.children.push(this.parseStruct());
            } else {
    ...
    parseStruct() {
        try {
            ...
            this.expectLex('}');
            this.expectLex(';');
            return { name: 'Struct', id, fields, children: fields };
        } catch(e) {
            this.sync(); return { name: 'Struct Error', isLeaf: true, isErrorNode: true };
        }
    }
    ...
    parseTopLevelFuncOrVar() {
        try {
            let typeParts = [];
            const typeKeywords = ['int', 'float', 'double', 'char', 'void', 'long', 'short', 'unsigned', 'signed', 'const'];
            
            let startPos = this.pos;
            let isFunc = false;
            
            while (this.peek() && (typeKeywords.includes(this.peek().lexeme) || this.peek().type === 'IDENTIFIER' || ['*'].includes(this.peek().lexeme))) {
                this.advance();
            }
    ...
    parseFunctionDecl() {
        let typeParts = [];
        const typeKeywords = ['int', 'float', 'double', 'char', 'void', 'long', 'short', 'unsigned', 'signed', 'const'];
        while (this.peek() && typeKeywords.includes(this.peek().lexeme)) {
            typeParts.push(this.advance().lexeme);
        }
        while (this.matchLex('*')) typeParts.push('*');
    ...
    parseParam() {
        let typeParts = [];
        while (this.peek() && ['int', 'float', 'double', 'char', 'void', 'long', 'short', 'unsigned', 'signed', 'const', 'struct'].includes(this.peek().lexeme)) {
            typeParts.push(this.advance().lexeme);
        }
        while (this.matchLex('*')) typeParts.push('*');
    ...
    parseStatement() {
        try {
            ...
            if (['int', 'float', 'double', 'char', 'void', 'long', 'short', 'unsigned', 'signed', 'const', 'struct'].includes(t.lexeme)) return this.parseVarDecl();
            if (t.lexeme === 'if') return this.parseIf();
            if (t.lexeme === 'while') return this.parseWhile();
            if (t.lexeme === 'do') return this.parseDoWhile();
            ...
            if (t.lexeme === 'goto') { this.advance(); let lbl = this.expectType('IDENTIFIER').lexeme; this.expectLex(';'); return { name: 'Goto', label: lbl, childen: [] }; }
            if (t.lexeme === 'printf') return this.parsePrintf();
            if (t.lexeme === 'scanf') return this.parseScanf();
            
            if (t.type === 'IDENTIFIER') {
                if (this.tokens[this.pos+1]?.lexeme === ':') {
                    let lbl = this.advance().lexeme; this.advance();
                    return { name: 'Label', id: lbl, isLeaf: true };
                }

                let start = this.pos;
                this.advance();
                while (this.matchLex('[') && this.parseExpression() && this.matchLex(']')) {}
                while (this.matchLex('.')) this.advance(); // struct access
                while (this.matchLex('->')) this.advance(); // pointer access
            ...
            this.sync();
            return { name: `Unhandled Statement`, isLeaf: true, isErrorNode: true };
        }
    }
    ...
    parseVarDecl() {
        let typeParts = [];
        let t = this.peek();
        while (t && ['int', 'float', 'double', 'char', 'void', 'long', 'short', 'unsigned', 'signed', 'const', 'struct'].includes(t.lexeme)) {
            typeParts.push(this.advance().lexeme);
            t = this.peek();
        }
        ...
        let parseSingle = () => {
            ...
            let node = { name: 'VarDecl', typeInfo: type + pStr, id, arraySize, children: [{ name: type+pStr, isLeaf: true }, { name: id, isLeaf: true }] };
            if (this.matchLex('=')) {
                node.expr = this.parseExpression();
                node.children.push(node.expr);
            }
            return node;
        };

        let nodes = [parseSingle()];
        while (this.matchLex(',')) nodes.push(parseSingle());
        this.expectLex(';');

        if (nodes.length === 1) return nodes[0];
        return { name: 'MultiVarDecl', children: nodes };
    }
    ...
    parseAssignment() {
        ...
        if (op === '++' || op === '--') {
            this.expectLex(';');
            return { name: 'AssignUn', id, op, children: [{name: id, isLeaf: true}] };
        }
    ...
    parsePrintf() {
        this.expectLex('printf'); this.expectLex('(');
        let args = [this.parseExpression()];
        while (this.matchLex(',')) args.push(this.parseExpression());
        this.expectLex(')'); this.expectLex(';');
        return { name: 'Printf', children: args };
    }

    parseScanf() {
        this.expectLex('scanf'); this.expectLex('(');
        let args = [this.parseExpression()];
        while (this.matchLex(',')) args.push(this.parseExpression());
        this.expectLex(')'); this.expectLex(';');
        return { name: 'Scanf', children: args };
    }

    parseIf() {
        this.expectLex('if'); this.expectLex('(');
        let cond = this.parseExpression(); this.expectLex(')');
    ...
    parseWhile() {
        this.expectLex('while'); this.expectLex('(');
        let cond = this.parseExpression(); this.expectLex(')');
        let body = this.parseStatement();
    ...
    parseDoWhile() {
        ...
        let body = this.parseStatement();
        this.expectLex('while'); this.expectLex('(');
        let cond = this.parseExpression(); this.expectLex(')'); this.expectLex(';');
        return { name: 'DoWhile', cond, body, children: [cond, body] };
    }
    ...
    parseFor() {
        ...
        let init = null;
        if (this.peek()?.lexeme !== ';') {
            if (['int', 'float', 'double'].includes(this.peek()?.lexeme)) init = this.parseVarDecl();
    ...
    parseExpressionStmt() {
        let expr = this.parseExpression();
        this.expectLex(';');
        return { name: 'ExprStmt', expr, children: [expr] };
    }
    ...
    parseFactor() {
        ...
        // Pointer Dereference or Address-Of
        if (this.matchLex('*')) { return { name: 'Deref', child: this.parseFactor(), children: [] }; }
        if (this.matchLex('&')) { return { name: 'AddressOf', child: this.parseFactor(), children: [] }; }
        ...
        if (['INT_CONST', 'FLOAT_CONST', 'STRING', 'CHAR_LITERAL', 'IDENTIFIER'].includes(t.type)) {
            ...
            if (node.type === 'IDENTIFIER' && this.matchLex('[')) {
                let index = this.parseExpression(); this.expectLex(']');
                return { name: `${node.lexeme}[]`, id: node.lexeme, index, children: [index] };
            }
            if (node.type === 'IDENTIFIER' && this.matchLex('(')) {
                let args = [];
                if (this.peek() && this.peek().lexeme !== ')') {
...
export function analyzeSemantics(ast) {
    ...
    function traverse(node) {
        ...
        if (node.name === 'Program') {
            ...
            logs.push(`C Preprocessor Header Linked: ${node.id}`);
        } else if (node.name === 'Struct') {
            let sym = { name: node.id, type: 'struct', scopeLevel: currentScopeLevel, mem: 'global' };
            symbolMapStack[symbolMapStack.length - 1].set(node.id, sym);
            flatSymbolTable.push(sym);
            logs.push(`Struct [${node.id}] context established`);
            ...
            traverse(node.body);
            exitScope();
        } else if (node.name === 'VarDecl' || node.name === 'MultiVarDecl') {
            if (node.name === 'MultiVarDecl') { node.children.forEach(traverse); return; }
            if (symbolMapStack[symbolMapStack.length - 1].has(node.id)) logs.push(`Warning: Duplicate C declaration [${node.id}]`);
            let memSize = node.arraySize ? 8 : 4;
            ...
            flatSymbolTable.push(sym);
            memOffset += memSize;
            logs.push(`Variable/Pointer [${node.id}] declared`);
            if (node.expr) traverse(node.expr);
        } else if (node.name.startsWith('Assign')) {
            let baseId = node.id.split('[')[0].split('.')[0].split('->')[0];
            if (!lookup(baseId)) logs.push(`Warning: Variable [${baseId}] assigned before declaration.`);
            if (node.expr) traverse(node.expr);
        } else if (node.name === 'Scanf') {
            logs.push(`I/O Operation: Memory capture bounded`);
        } else {
...
export function generateICG(ast) {
    ...
    function genExpr(node) {
        ...
        if (node.name === 'Deref' || node.name === 'AddressOf') return '*' + genExpr(node.child);
    ...
    function genStmt(node) {
        ...
        if (node.name === 'Program' || node.name === 'Block') {
            ...
            if (node.name === 'MultiVarDecl') node.children.forEach(genStmt);
            else if (node.expr) emit('=', genExpr(node.expr), '', node.id);
        } else if (node.name === 'Assign') {
            emit('=', genExpr(node.expr), '', node.id);
        } else if (node.name === 'AssignUn') {
            let t = newTemp(); emit(node.op === '++' ? '+' : '-', node.id, '1', t); emit('=', t, '', node.id);
            ...
            emit('call', 'scanf_sys', node.children.length, '');
...
// --- Diagram Generation ---
export function getCDiagrams() {
    return {
        dfa: `stateDiagram-v2\n[*] --> S0\nS0 --> S1 : [a-zA-Z_]\nS1 --> S1 : [a-zA-Z0-9_]\nS1 --> [*] : Identifier/Keyword`,
        nfa: `graph LR\nS((Start)) -->|digit| N(Number)\nS -->|letter| I(Ident)\nI --> End`
    };
}
